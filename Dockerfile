...
RUN envsubst '${ICLOUD_API_BASE_URL}' < umc/lib/constants.ts > src/lib/constants_umc.ts && \
    ...
    cp umc/lib/Image.svelte src/lib/components/common/Image.svelte && \
    mkdir -p src/lib/apis/umc && \
    cp umc/lib/images.ts src/lib/apis/images/index.ts && \
...
RUN pip3 install --no-cache-dir uv && \
    ...
    # Solve NameError: name 'init_empty_weights' is not defined
