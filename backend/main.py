...
from apps.umc.main import (
    app as umc_app,
    generate_chat_completion as generate_umc_chat_completion,
)
...
def get_task_model_id(default_model_id):
    ...
    if app.state.MODELS[task_model_id]["owned_by"] == "umc":
        # 這個一定要設定成 umcgpt-4o，不然會有問題。 Arvin Yang - 2024/08/23
        return "umcgpt-4o:latest"
    elif app.state.MODELS[task_model_id]["owned_by"] == "ollama":
        if (
...
async def chat_completion_filter_functions_handler(body, model, extra_params):
    ...
    for filter_id in filter_ids:
        ...
        if filter_id in webui_app.state.FUNCTIONS:
            ...
            print(1, function_module)
        else:
            function_module, _, _ = load_function_module_by_id(filter_id)
            webui_app.state.FUNCTIONS[filter_id] = function_module
            print(2, function_module)
...
async def get_content_from_response(response) -> Optional[str]:
    ...
    if hasattr(response, "body_iterator"):
        async for chunk in response.body_iterator:
            chunk_str = chunk.decode('utf-8').replace('data: ', '').replace('\n', '')
            if chunk_str == "[DONE]" or chunk_str == "":
                break

            data = json.loads(chunk_str)
            print(f"get_content_from_response: {data}")
        ...
        print(f"get_content_from_response: {response}")
...
async def chat_completion_tools_handler(
    ...
    # 把 tools 轉成 {role:xx, content:xx} 的格式。
    ...
    try:
        # 把 tools' payload 拿去問 AI。 Arvin Yang - 2024/08/23
        response = await generate_chat_completions(form_data=payload, user=user)
        log.debug(f"{response=}")
        # 擷取 AI 的回答。 Arvin Yang - 2024/08/23
        ...
        # 因為 content 有可能是 json 格式，所以要先解析。 Arvin Yang - 2024/08/23
        content = content.replace("```json", "").replace("```", "")

        result = json.loads(content)

        # 如果 result 不是 dict，就直接回傳，不然會報錯。 Arvin Yang - 2024/08/23
...
def is_chat_completion_request(request):
    return request.method == "POST" and any(
        ...
        for endpoint in ["/ollama/api/chat", "/chat/completions", "/umc/api/v1/api/chat", "/umc/api/v1/chat/completions"]
    )

def is_completion_request(request):
    return request.method == "POST" and any(
        endpoint in request.url.path
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if is_chat_completion_request(request):
            ...
            if isinstance(response, StreamingResponse):
                ...
                # application/json 是 UMC 的格式，text/event-stream 是 OpenAI 的格式。 Arvin Yang - 2024/08/23
                content_type = response.headers.get("Content-Type")
                is_openai = "text/event-stream" in content_type or "application/json" in content_type
        ...
        elif is_completion_request(request):
            # 先解包 request.body
            body = await request.body()
            body_str = body.decode("utf-8")
            body = json.loads(body_str) if body_str else {}
    ...
    async def _dispatch(self, request: Request, call_next):
        ...
        if "info" in model and "meta" in model["info"] and "toolIds" in model["info"]["meta"]:
            if "tool_ids" not in metadata or metadata["tool_ids"] is None:
                metadata["tool_ids"] = model["info"]["meta"]["toolIds"]
            else:
                metadata["tool_ids"].extend(model["info"]["meta"]["toolIds"])
        ...
        if "info" in model and "meta" in model["info"] and "knowledge" in model["info"]["meta"]:
            if "files" not in metadata or metadata["files"] is None:
        ...
        if len(contexts) > 0:
            ...
            if "owned_by" in model and model["owned_by"] == "ollama":
                ...
                if any(
                    endpoint in request.url.path
                    for endpoint in ["/umc/api/v1/api/chat", "/umc/api/v1/chat/completions"]
                ) or model.get("owned_by", None) == "umc":
...
def filter_pipeline(payload, user):
    ...
    if "messages" in new_payload:
        new_payload["messages"] = inverse_refactor_messages(new_payload["messages"])

    if "pipeline" in model:
        print(f"model {model_id} has pipeline")
...
async def get_all_models():
    ...
    models = pipe_models + openai_models + ollama_models + umc_app.state.MODELS
