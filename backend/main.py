    ...
    async def dispatch(self, request: Request, call_next):
        ...
        async def stream_wrapper(original_generator, data_items):
            for item in data_items:
                print("item", wrap_item(json.dumps(item)))
...
class CompletionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        return StreamingResponse(self.stream_wrapper(response.body_iterator))
    ...
    async def stream_wrapper(self, original_generator):
        async for data in original_generator:
            try:
                ...
                # 把 choices 裡面的 delta.content 轉成 text。 Arvin Yang - 2024/08/29
                ...
                # 再把 json_data 轉回 bytes。 Arvin Yang - 2024/08/29
...
app.add_middleware(CompletionMiddleware)
# endregion
